...
- `platform-build.yml`: builds ENDR backend (`endr-api`) and frontend (`endr-case`) images; PRs run build-only validation, while `main` builds/pushes to Docker Hub and persists chart image tags.
