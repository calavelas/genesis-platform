...
- On PR/main changes for ENDR frontend/backend paths, workflow `platform-build.yml` builds `endr-api` + `endr-case`; PRs validate build only, and `main` pushes images then updates `ENDR/charts/endr-api|endr-case/values.yaml` with the new `git-<sha>` tag.
