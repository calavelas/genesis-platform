...
def _collect_commit_files(
    ...
    for file_path in sorted(stage_service_dir.rglob("*")):
        ...
        repo_path = f"services/{service_name}/{relative}"
    ...
    for file_path in sorted(stage_gitops_dir.rglob("*")):
        ...
        if relative == "argocd-application.yaml":
            repo_path = f"platform/clusters/local/apps/{service_name}.yaml"
        else:
            repo_path = f"services/{service_name}/chart/{relative}"
        files[repo_path] = file_path.read_bytes()
...
def create_service(request: CreateServiceRequest) -> CreateServiceResponse:
    ...
    if not service_template_name or not gitops_template_name:
        raise ValueError("service and gitops templates must be configured in idp-config.yaml")

    for existing in services_config.services:
        if existing.name == request.name:
            raise ValueError(f"service already exists in services-config.yaml: {request.name}")
    ...
    try:
        ...
        raise ValueError("services-config.yaml must be inside repository root for PR workflow") from exc
    commit_files[relative_services_config] = _render_updated_services_config(services_config_path, service)

    preview_config_file = staging_root / "services-config.yaml"
